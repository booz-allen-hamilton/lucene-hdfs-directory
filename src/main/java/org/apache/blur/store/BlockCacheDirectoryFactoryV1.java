...
import static org.apache.blur.util.BlurConstants.BLUR_SHARD_BLOCKCACHE_DIRECT_MEMORY_ALLOCATION;
import static org.apache.blur.util.BlurConstants.BLUR_SHARD_BLOCKCACHE_SLAB_COUNT;
...
public class BlockCacheDirectoryFactoryV1 extends BlockCacheDirectoryFactory {
  ...
  public BlockCacheDirectoryFactoryV1(Configuration configuration, long totalNumberOfBytes) {
    ...
    if (slabCount >= 1) {
      ...
      LOG.info(MessageFormat.format("Number of slabs of block cache [{0}] with direct memory allocation set to [{1}]", slabCount,
          directAllocation));
      LOG.info(MessageFormat.format("Block cache target memory usage, slab size of [{0}] will allocate [{1}] slabs and use ~[{2}] bytes",
